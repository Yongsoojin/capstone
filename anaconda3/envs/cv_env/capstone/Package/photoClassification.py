...
def classification(imgURL, tripURL, place, time) :
    ...
    # 날짜별로 분류 진행
    folder2save = tripURL + '/' + time + '/' + place
    if not os.path.exists(folder2save) :
        os.makedirs(folder2save)

    # classification DB 확인 후 채우기
    # data가 이미 저장된 것인지 확인
    checkclassification = select.select('classification', 'trip_url', f"maingroup = '{time}' AND subgroup = '{place}'")
    # print('checkclassification : ', checkclassification)
    if len(checkclassification) == 0 :
        # classifiation DB 채우기
        insert.insert('classification', '(maingroup, subgroup, trip_url)', f"('{time}', '{place}', '{tripURL}')")
