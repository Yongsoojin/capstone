
## PHOTOCATE - 메타데이터를 기반으로 한 사진 분류와 필터처리 효과를 통한 사진편집을 할 수 있는 웹사이트

<br><br>
<p align="center">
<img src="https://github.com/Yongsoojin/capstone/assets/87532431/97dfdfdd-a100-420a-9482-b49bc8bd65c1.jpg" width="300" height="300" />
<br><br>

</p>
<br><br>

## 📱 프로젝트 소개
<br>
<p>
  포토카테는... <br>
  사용자가 로그인을 한 후, Photo Categorize[사진분류] 탭에서 사진을 업로드하면 해당 사진 내 메타데이터를 이용하여 장소별, 시간별 분류가 가능합니다. <br><br>

  또한 Photo Edit 탭을 통하여 개별 사진을 필터처리 할 수 있습니다. <br>
  필터효과에는 모자이크, 블러, 색필터가 있으며 모자이크와 블러는 자동,수동 모드가 가능하며, 색필터는 현재까지 흑백필터가 가능합니다. <br><br>
  

 카카오톡에서 친구들과 여행 사진을 공유하였을 때 장소별, 시간별에 의해 보기좋게 분류된다면 <br>
 사용자에게 편의성을 줄 수 있을 것 같고, SNS에 업로드 할 시 초상권 문제에 대비해 간편하게 편집을 할 수 있으면 좋겠다는 취지에서 고안되었습니다.<br>
 </p>

<br><br>
<br><br>

## 🏅 프로젝트 구성원

|포지션|프로필|이름|
|----|-----|---|
|프론트엔드|<img src="https://github.com/puputia/Ani-wana/assets/87532431/0707964b-b44a-45df-8ea2-ad65348106be.jpg" width="200" height="200" />|서혜인|
|백엔드|<img src ="https://github.com/Yongsoojin/capstone/assets/87532431/b58decbb-135b-4555-80aa-ad91f5ef06ee.jpg" width="200" height="200"/>|용수진|
 
<br><br>




## 핵심 기능 <br>
  + <strong>[구현 완료]</strong><br>
  💡 홈페이지 : 첫 화면, 프로젝트 소개와 Photo categorize[사진분류]탭과 Photo Edit[사진편집] 탭의 사용법 기재, 결과화면 슬라이더 및 예시<br>
  💡 Photo Categorize[사진분류]: 사용자가 이미지가 담겨있는 폴더를 업로드하면 해당 폴더를 장소별, 시간별에 따라 분류해줌<br>
  💡 Photo Edit[사진편집]: 사용자가 개별 사진업로드 페이지에서 이미지를 업로드하면 해당 사진을 편집할 수 있게해줌, Photo Categorize에서 분류된 이미지를 클릭하면
Photo Edit 페이지로 자동이동하며 편집이 가능함.<br>
  💡 로그인: 로그인이 가능하며 추후 Photo Categorize에 공유폴더 생성 가능   <br>
  💡 개별 사진 업로드 페이지: 개별 이미지 파일 업로드 가능   <br>
  💡 폴더 업로드 페이지: 이미지가 들어 있는 폴더 파일 업로드 가능   <br>
  + <strong>[실력을 쌓은 후 추후 유지보수 예정]</strong><br>
  💡 node.js를 통한 배포 & 백엔드와 프론트엔드 연결<br>



<br><br>

## 기술 스택
 <p align="center">
   <img src="https://img.shields.io/badge/Javascript-ffb13b?style=flat-square&logo=javascript&logoColor=white"/></a>&nbsp 
   <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=white"/>&nbsp
</p>
<br><br>

## 협업 커뮤니케이션
+ <a href="https://github.com/Yongsoojin/capstone/tree/hyein">Github (링크 이동)</a>
+ Disord 


<br><br>

## 📹 시연 영상 
<p align="center
  <img width="1280" alt="사진편집페이지" src="https://github.com/Yongsoojin/capstone/assets/87532431/19cd030f-828c-4612-9a7a-aa0bf96a37e7">
</p>
 
<br><br>


## 각 페이지 
### 1. 홈페이지
![메인페이지](https://github.com/Yongsoojin/capstone/assets/87532431/939b4260-c453-40bc-8c69-acc362ece361)<br/>



<br/><br/>

### 2. 로그인 페이지
![로그인 페이지](https://github.com/Yongsoojin/capstone/assets/87532431/73dc12b0-3b3a-4615-ac44-d99afb892ff0) <br/>
 <br/><br/>

### 3. Photo Categorize(사진분류)
![공유폴더페이지-공유폴더](https://github.com/Yongsoojin/capstone/assets/87532431/2fcd2abe-0b52-4e99-a256-4b245707e2e3) <br/>
![공유폴더페이지-사용자폴더](https://github.com/Yongsoojin/capstone/assets/87532431/89b8b9df-48ad-4049-a6f6-1e5a30686569) <br/>
![공유폴더페이지-장소별분류](https://github.com/Yongsoojin/capstone/assets/87532431/228e223f-827a-40b4-bc61-cf0d25f59eb0) <br/>
![공유폴더페이지-장소시간](https://github.com/Yongsoojin/capstone/assets/87532431/41392d86-e89e-477f-a07d-53048e7c3030) <br/>
![공유폴더페이지-장소이미지](https://github.com/Yongsoojin/capstone/assets/87532431/d11cb7e5-d527-4299-a9a3-37aae405f13f) <br/>
<br/><br/>

### 4. Photo Edit(사진편집)
<img width="1280" alt="사진편집페이지" src="https://github.com/Yongsoojin/capstone/assets/87532431/10530238-a04c-4fc4-ab2c-b8dc63a4b9bc"> <br/>


### 5. 폴더 이미지업로드 페이지
![공유폴더페이지-이미지업로드](https://github.com/Yongsoojin/capstone/assets/87532431/0fbd472f-961e-4976-863e-ae90df42cbd3)

### 6. 개별 이미지업로드 페이지
![개별사진편집페이지](https://github.com/Yongsoojin/capstone/assets/87532431/8eac7287-9ad9-4d25-b543-ffd2b7b648f1)




Copyright ©PHOTOCATE 2023. All Rights Reserved.
